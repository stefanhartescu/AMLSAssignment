...
def classify_age(test_samples):
    ...
    Y_pred_te = SVM_age(Xtrain, Ytrain, Xtest) #trains the model
    size_te = len(Y_pred_te)
    ...
    print(precision / size_pred)
    percentage = [precision / size_pred]
    np.savetxt("task_2precision.csv", percentage, delimiter=',') #creates csv files for predicted values & precision
    a = pd.read_csv("noise_classified.csv")
...
def classify_eyeglasses(test_samples):
    ...
    Y_pred_te = SVM_eyeglasses(Xtrain, Ytrain, Xtest) #trains the model
    size_te = len(Y_pred_te)
    ...
    print(precision / size_pred)
    percentage = [precision / size_pred]
    np.savetxt("task_3precision.csv", percentage, delimiter=',') #creates csv files for predicted values & precision
    a = pd.read_csv("noise_classified.csv")
...
def classify_human(test_samples):
    ...
    Y_pred_te = SVM_human(Xtrain, Ytrain, Xtest) #trains the model
    size_te = len(Y_pred_te)
    ...
    print(precision / size_pred)
    percentage = [precision / size_pred]
    np.savetxt("task_4precision.csv", percentage, delimiter=',') #creates csv files for predicted values & precision
    a = pd.read_csv("noise_classified.csv")
...
def classify_smile_dtree(test_samples):
    ...
    Xtest = Xtest.reshape(Xtest.shape[0], Xtest.shape[1] * Xtest.shape[2])
    Ytrain, Ytest = get_labels(3, test_samples)
    Y_pred_te = DTree_smile(Xtrain, Ytrain, Xtest) #trains the model
    size_te = len(Y_pred_te)
    ...
    print(precision / size_pred)
    percentage = [precision / size_pred]
    np.savetxt("task_1dtreeprecision.csv", percentage, delimiter=',') #creates csv files for predicted values & precision
    a = pd.read_csv("noise_classified.csv")
...
def classify_age_dtree(test_samples):
    ...
    Xtest = Xtest.reshape(Xtest.shape[0], Xtest.shape[1] * Xtest.shape[2])
    Ytrain, Ytest = get_labels(4, test_samples)
    Y_pred_te = DTree_age(Xtrain, Ytrain, Xtest) #trains the model
    size_te = len(Y_pred_te)
    ...
    print(precision / size_pred)
    percentage = [precision / size_pred]
    np.savetxt("task_2dtreeprecision.csv", percentage, delimiter=',') #creates csv files for predicted values & precision
    a = pd.read_csv("noise_classified.csv")
...
def classify_eyeglasses_dtree(test_samples):
    ...
    Xtest = Xtest.reshape(Xtest.shape[0], Xtest.shape[1] * Xtest.shape[2])
    Ytrain, Ytest = get_labels(2, test_samples)
    Y_pred_te = DTree_eyeglasses(Xtrain, Ytrain, Xtest) #trains the model
    size_te = len(Y_pred_te)
    ...
    print(precision / size_pred)
    percentage = [precision / size_pred]
    np.savetxt("task_3dtreeprecision.csv", percentage, delimiter=',') #creates csv files for predicted values & precision
    a = pd.read_csv("noise_classified.csv")
...
def classify_human_dtree(test_samples):
    ...
    Xtest = Xtest.reshape(Xtest.shape[0], Xtest.shape[1] * Xtest.shape[2])
    Ytrain, Ytest = get_labels(5, test_samples)
    Y_pred_te = DTree_human(Xtrain, Ytrain, Xtest) #trains the model
    size_te = len(Y_pred_te)
    ...
    print(precision / size_pred)
    percentage = [precision / size_pred]
    np.savetxt("task_4dtreeprecision.csv", percentage, delimiter=',') #creates csv files for predicted values & precision
    a = pd.read_csv("noise_classified.csv")
...
def classify_haircolor_dtree(test_samples):
    ...
    Y_pred_te = DTree_haircolor(Xtrain, Ytrain*2, Xtest) #trains the model
    size_te = len(Y_pred_te)
    Y_pred_tr = DTree_haircolor(Xtrain, Ytrain*2, Xtrain)
    size_tr = len(Y_pred_tr)
    test_accuracy = accuracy_score(Ytest*2, Y_pred_te) * 100 #prints the accuracy found on training/testing data
    ...
    print(precision / size_pred)
    percentage = [precision / size_pred]
    np.savetxt("task_5dtreeprecision.csv", percentage, delimiter=',') #creates csv files for predicted values & precision
    a = pd.read_csv("noise_classified.csv")
...
def classify_haircolor_KNN(test_samples):
    ...
    Xtest = Xtest.reshape(Xtest.shape[0], Xtest.shape[1] * Xtest.shape[2])
    Ytrain, Ytest = get_labels(1, test_samples)
    Y_pred_te = KNN_haircolor(Xtrain, Ytrain*2, Xtest) #trains the model
    size_te = len(Y_pred_te)
    ...
    print(precision / size_pred)
    percentage = [precision / size_pred]
    np.savetxt("task_5KNNprecision.csv", percentage, delimiter=',') #creates csv files for predicted values & precision
    a = pd.read_csv("noise_classified.csv")
